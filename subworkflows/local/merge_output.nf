...
workflow MERGE_OUTPUT {
    ...
    if ( params.merge_output ) {
        ...
        .groupTuple( by: [0] )
        .map { meta, runs, read_counts, bams -> [meta + [id: meta.specimen + ".merged_${params.merge_output}", merge_source: runs.sort().join("\n") + "\n", read_count: read_counts.sum()], bams.sort { it.toString() }] }
