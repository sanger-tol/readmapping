...
workflow CRAM_MAP_ILLUMINA_RNA {
    ...
    ch_assemblies
        .map { meta, _sample ->
            if (!val_asm_meta_list.add(meta)) {
                error("Error: Duplicate meta object found in `ch_assemblies` in CRAM_MAP_ILLUMINA_RNA: ${meta}")
            }
            meta
        }
    ...
    //
    ch_rna_cram_meta_mod = ch_rna_cram
        .transpose()
        .map { meta, cram -> [ meta + [ cramfile: cram ], cram ]}

    ch_rna_cram_raw = ch_rna_cram_meta_mod
            .branch { meta, cram ->
    ...
    //
    SAMTOOLS_INDEX(ch_rna_cram_raw.no_index)

    ch_rna_cram_indexed = ch_rna_cram_raw.have_index
        .mix(
            ch_rna_cram_raw.no_index.join(SAMTOOLS_INDEX.out.index)
        )
    ...
    //
    SAMTOOLS_SPLITHEADER(ch_rna_cram_meta_mod)
    ...
    if(val_aligner == "star") {
        ...
        ch_mapping_inputs = ch_cram_rg
            ...
            .combine(STAR_GENOMEGENERATE.out.index, by: 0)
        ...
        ch_mapped_bams = STAR_ALIGN.out.bam
    } else {
