...
```console
nextflow run nf-core/readmapping --input samplesheet.csv --genome GRCh37 -profile docker
```
...
First, go to the [nf-core/readmapping releases page](https://github.com/nf-core/readmapping/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
