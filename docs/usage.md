...
| `datafile` | Full path to read data file. Must be `bam` or `cram` for `hic`. Must be `bam` or `cram` or `fastq.gz` or `fq.gz` for `Illumina`. Must be `bam` for `pacbio`. Must be `fastq.gz` or `fq.gz` for `ont`.                                                                                   |
